...
def save_fig(i, path="results/vanilla_vae"):
...
def train(epochs=50, batch_size=16, z_dim=100):
  ...
  with tf.Session(graph=graph) as sess:
    var_dict = model.build_graph(z_dim=z_dim)
    model.init(sess)
    for e in tqdm(range(epochs)):
      for b in range(batches_per_epoch):
        X, _ = mnist.train.next_batch(batch_size)
        X.reshape([-1, 784])
        loss = model.train(sess, var_dict, X)
      ...
      samples = model.sample(sess, var_dict, z)
